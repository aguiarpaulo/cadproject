...
from src.schemas.data_models import RawInputSchema, StructuredPlanSchema
...
class Orchestrator:
    ...
    def run(self, text_input: str, output_filename: str = "output.dxf"):
        ...
        # 3. Handle feedback loop if necessary
        while isinstance(plan_or_question, str):
            print("\n--- Clarification Required ---")
            user_response = input(f"Agent: {plan_or_question}\nYou: ")
            
            # Create a new, combined input for the parser
            combined_input = text_input + " " + user_response
        ...
        if not isinstance(plan_or_question, StructuredPlanSchema):
             print("Could not derive a valid plan. Aborting.")
             return
